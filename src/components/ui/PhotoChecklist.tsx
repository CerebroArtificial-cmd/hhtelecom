...
const RT_FIELDS: Array<{ id: string; label: string; requireCoords?: boolean; multiple?: boolean; minCount?: number; fixedSlots?: number; }> = [
  ...
  { id: "rt_espaco_medidor_1", label: "ESPAÇO PARA O MEDIDOR 1" },
  { id: "rt_espaco_medidor_2", label: "ESPAÇO MEDIDOR 2" },
  ...
  { id: "rt_area_equip_1", label: "ÁREA DOS EQUIPAMENTOS 1" },
  { id: "rt_area_equip_2", label: "ÁREA DOS EQUIPAMENTOS 2" },
  { id: "rt_area_equip_3", label: "ÁREA DOS EQUIPAMENTOS 3" },
  { id: "rt_energia_topo_1", label: "ENERGIA NO TOPO 1" },
  { id: "rt_energia_topo_2", label: "ENERGIA NO TOPO 2" },
  { id: "rt_caixa_dagua_1", label: "CAIXA D'ÁGUA 1" },
  { id: "rt_caixa_dagua_2", label: "CAIXA D'ÁGUA 2" },
  { id: "rt_interna_caixa_dagua_1", label: "INTERNA CAIXA D'ÁGUA 1" },
  { id: "rt_interna_caixa_dagua_2", label: "INTERNA CAIXA D'ÁGUA 2" },
...
function buildRenamedFile(file: File, photoId: string) {
  const name = file.name || "foto.jpg";
  const ext = name.includes(".") ? name.split(".").pop() : "jpg";
  const newName = `${photoId}.${ext}`.replace(/\.+/g, ".");
  try {
    return { file: new File([file], newName, { type: file.type || "image/jpeg" }), name: newName };
  } catch {
...
export default function PhotoChecklist({ data, onChange, onAutoSave }: PhotoChecklistProps) {
  ...
  const onFiles = async (id: string, fileList: FileList | null, minCount?: number) => {
    ...
    for (const f of files) {
      ...
      const renamed = buildRenamedFile(compressed, photoId);


      processed.push(renamed.file);
  ...
  const onFileForAngle = async (id: string, angleIndex: number, fileList: FileList | null) => {
    ...
    const renamed = buildRenamedFile(processed, photoId);


    currentFiles[angleIndex] = renamed.file;
  ...
  const onFileForSlot = async (id: string, slotIndex: number, fileList: FileList | null) => {
    ...
    currentFiles[slotIndex] = renamed.file;
